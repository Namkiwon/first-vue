  ...
  watch:{
    valueInputx(){
      if(this.visible){
        ...
        let valuex = valueInputx[valueInputx.length-1] //어떤 키워드로 아이템을 하나라도 찾았을 때의 키워드
        var re = new RegExp(valuex,"i");

        var str = ""
        var originText = this.vsText
        let searchList = valuex.split("")

        var idx = 0;
        for(var i = 0; i < searchList.length; i++){
          console.log(originText.length + "333333")
          for(var j=idx; j< originText.length; j++){
            if(originText.charAt(j) === searchList[i] && originText.charAt(j)!= " ") {
              var b = `<span class="searchx">${originText.charAt(j)}</span>`
              originText = originText.substring(0,j) + b + originText.substring(j+1,originText.length)
              idx = j+b.length;
              console.log(b.length+"45")
              console.log(originText.length + "45")
              break;
            }
          }
        }
